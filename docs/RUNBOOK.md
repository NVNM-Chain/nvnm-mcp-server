...
- **Set `NVNM_TRUSTED_PROXY_HOPS` to the real number of proxy hops in front of the server** (`1` = single ingress; `2` = CDN + ingress). A value that is too *high* is safe (falls back to `RemoteAddr`); a value that is too *low* under-collapses but never trusts a forged value. A value that does not match reality either fails to protect (too high, effectively collapsing everyone to a proxy IP) or, if it matches an attacker-controlled hop, defeats the control — see `DATA_HANDLING.md` § 5 for the full failure-mode table.
