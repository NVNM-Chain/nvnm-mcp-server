# Automated Testing — Suite and CI Gates

This document describes the automated testing strategy and framework for the NVNM Chain MCP Server. Current test results live in CI, not here.

For **manual end-to-end testing** — running the server locally and driving it from a real MCP client (Claude Code, Claude Desktop, a claude.ai custom connector, or the Messages API MCP connector) — see [`TESTING.md`](TESTING.md).
