...
| `index`         | `uint64` | **required** | Version index of the record to update, 1-based (the first version is 1). There is no "latest" default -- index 0 is rejected on chain. `anchor_get_records` reports the index of each version. |
