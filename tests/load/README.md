...
- **Authentication**: the current k6 script does **not** send `Authorization` headers, and running with no keys configured is **not** an option — the HTTP transport fails closed at boot with `ErrHTTPAuthRequired` when neither `MCP_API_KEYS_FILE` (holding an enabled key) nor `MCP_API_KEY` is set (see [`docs/TESTING.md`](../../docs/TESTING.md) § 2). Every scenario below calls read-only tools, so configure keyless reads instead: in `.env`, keep a validator configured (`MCP_API_KEYS_FILE=.mcp-keys.json` with at least one enabled key — `make key-create NAME=load-test ROLES=reader` creates one — or `MCP_API_KEY` + `MCP_API_KEY_ROLES`) **and** set `MCP_KEYLESS_READS=true` so the anonymous k6 requests are admitted on the read path. If you need authenticated load testing, extend `k6_mcp_http.js` to read a token from `__ENV.MCP_AUTH_TOKEN` and add it to the request headers in `setup()` and the VU code.
